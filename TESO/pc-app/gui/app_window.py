...
class MainWindow(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        # Crear fondo animado para la selección
        # Lo creamos ANTES de los botones para que quede por debajo
        self.selection_bg = ctk.CTkFrame(self.sidebar_frame, width=180, height=40, corner_radius=20, fg_color="#B026FF") # Color vibrante para que coincida con el gradiente
        self.selection_bg.place(x=20, y=-100) # Inicialmente oculto fuera de pantalla
    ...
    def _reset_nav_buttons(self):
        for btn in [self.btn_orders, self.btn_menu, self.btn_payments, self.btn_storage, self.btn_settings]:
            btn.configure(text_color="#A0A0A0")

    def _set_active_nav_button(self, button):
        self._reset_nav_buttons()
        # Cambiar el texto a blanco para el botón activo
        button.configure(text_color="#FFFFFF")
        
        # Iniciar animación del fondo
        self._animate_selection(button)

    def _animate_selection(self, target_btn):
        # Asegurar que el layout se haya actualizado para tener las coordenadas correctas
        self.update_idletasks()
        
        target_y = target_btn.winfo_y()
        current_y = int(self.selection_bg.place_info().get('y', -100))
        
        if current_y == -100:
            # Salto instantáneo en la primera carga
            self.selection_bg.place(x=20, y=target_y)
            return

        # Animación fluida
        def step_animation():
            curr_y = int(self.selection_bg.place_info().get('y', target_y))
            ...
            # Mover un 20% de la distancia restante cada vez
            next_y = curr_y + (diff * 0.3)
            self.selection_bg.place(x=20, y=next_y)
            self.after(16, step_animation) # ~60fps
