...
public class NotificationService extends NotificationListenerService {
    ...
    private void sendReply(StatusBarNotification sbn) {
        ...
        String fallbackReplyText = replyText;
        ...
        if (senderTitle == null || senderTitle.trim().isEmpty()) {
            senderTitle = "WhatsApp Contact";
        }

        // Prioridad: Servidor PC (Cerebro IA Local)
        if (preferencesManager.isPcServerEnabled() && incomingMessage != null && !incomingMessage.trim().isEmpty()) {
            ...
            PcServerService.getInstance().sendWebhookMessage(
                ...
                finalMsg,
                sbn.getPackageName(),
                new PcServerService.WebhookCallback() {
                    @Override
                    ...
                    @Override
                    public void onError(String errorMessage) {
                        Log.w(TAG, "Error conectando con servidor PC: " + errorMessage + ". Usando fallback.");
                        sendActualReply(sbn, notificationWear, fallbackReplyText);
                    }
                }
            );
